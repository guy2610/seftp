...
`plot_results.py` turns run reports into graphs for latency, throughput, outcome rates, CPU, RSS, upload-size comparisons, and mixed-workload per-operation comparisons. The plotting layer can continue to evolve, but the comparison CLI already gives a lightweight way to explain Stage 8 changes directly from JSON artifacts.
